...
SAME_GROUP_BASE_SIMILARITY = 0.75  # 同一子类组的默认相似度

# 跨组相似度特例（override）：某些材料虽不同组但有语义关联
CROSS_GROUP_OVERRIDES: Dict[str, Dict[Tuple[str, str], float]] = {
    "wall_material": {
        ("大理石", "瓷砖"): 0.35,    # 都是硬质板材但纹理差异大（天然vs规则）
        ("墙纸", "乳胶漆"): 0.3,     # 贴面 vs 涂料 — 稍有关联
        ("木饰面", "软硬包"): 0.35,  # 木质vs软质，都有分块视觉效果
        ("艺术漆", "乳胶漆"): 0.5,   # 都有纯色效果，艺术漆多了肌理
        ("玻璃", "石膏"): 0.15,      # 透明vs立体造型，差异大
    },
    "floor_material": {
        ("实木地板", "石塑地板"): 0.65,  # 视觉相似但纹理重复性不同
        ("实木地板", "防腐木"): 0.55,    # 都是木板效果，但防腐木更粗糙风化
        ("地砖", "大理石"): 0.35,        # 规则砖缝vs天然流动纹路，差异明显
        ("微水泥", "地砖"): 0.2,          # 无缝vs网格，差异大
        ("微水泥", "涂料顶面"): 0.15,     # 跨字段比较无意义，保留低值
        ("地毯", "所有其他"): 0.05,       # 绒毛纤维独特，几乎无相似
    },
    "ceiling_material": {
        ("石膏板吊顶", "涂料顶面"): 0.55,  # 视觉上都是平整白色顶面
        ("金属吊顶", "石膏板吊顶"): 0.4,   # 都有吊顶造型，金属更规则
        ("木饰吊顶", "石膏板吊顶"): 0.35,  # 都有造型但材质不同
        ("石膏线条", "涂料顶面"): 0.25,    # 线条vs平顶，关联弱
        ("玻璃顶", "所有其他"): 0.1,       # 透明透光特性独特
    },
    "space_type": {
        ("客厅", "休闲区"): 0.45,          # 都有休闲座椅，布局风格接近
        ("卧室", "书房"): 0.35,            # 都属安静私密空间，但家具差异大
        ("卧室", "储物间"): 0.15,           # 都有收纳但功能完全不同
        ("走廊", "阳台"): 0.2,             # 都是通道但开放vs封闭差异大
        ("厨房", "卫生间"): 0.3,            # 都有橱柜台面+瓷砖，功能不同
        ("客厅", "餐厅"): 0.4,             # 都属公共活动区，家具不同
        ("储物间", "休闲区"): 0.1,          # 功能完全相反
    },
}
