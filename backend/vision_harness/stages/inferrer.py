...
def build_full_prompt() -> str:
    """构建全图结构化识别 Prompt（十二大类枚举约束，不引入子类型）"""
    return f"""你是一位专业的室内装修材料识别专家。请识别这张家装效果图，**仅提取以下4项信息**，所有取值必须从下方给定列表中**严格选择**，禁止自行编造、造词、拼写错误。

特别注意：空间类型只输出十二大类（客厅、餐厅、卧室、书房、走廊、阳台、厨房、卫生间、门厅、衣帽间、储物间、休闲区），即使能区分主卧/次卧、主卫/客卫、西厨/中厨、休闲阳台/生活阳台等子类型，也请一律归入大类，禁止输出任何不在列表中的子类型名称。
